...
const CycleGeneratorForm: FunctionComponent<cycleGeneratorFormEntries> = ({
  ...
  const { setTopicTasksList } = React.useContext(
    TopicTasksContext
  )! as TopicTasksContextType;

  const [selectedTopicB, setSelectedTopicB] = React.useState<ITopic | null>(
    null
  );
  const [selectedTopicC, setSelectedTopicC] = React.useState<ITopic | null>(
    null
  );

  function setTopicB(topicId: Number | null) {
    const topic: ITopic | null =
      topics.find((topic) => topic.topicId === topicId) || null;
    ...
    return (
      <>
        ...
        <Select
          ...
          label="Tópico B"
          onChange={(event) =>
            setTopicB(parseInt(event.target.value!.toString()))
          }
        >
          {topicsB.map((topic) => (
    ...
    return (
      <>
        ...
        <Select
          ...
          label="Tópico C"
          onChange={(event) =>
            setTopicC(parseInt(event.target.value!.toString()))
          }
        >
          {topicsC.map((topic) => (
            <MenuItem value={topic.topicId}>{topic.topicName}</MenuItem>
  ...
  async function generateCycle() {
    ...
    cycle.forEach((topic) => {
      topicsApi.push({
        ...
        score: calcScoreTopic(
          topic.totalCorrectQuestion,
          topic.totalDoneQuestion
        ),
      });
    });
  ...
  return (
    <div>
      ...
      <Button onClick={generateCycle}>Gerar</Button>
    </div>
  );
};

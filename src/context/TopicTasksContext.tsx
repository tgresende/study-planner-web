...
export const TopicTasksContext =
  React.createContext<TopicTasksContextType>(defaultState);
...
const TopicTasksProvider: React.FC = ({ children }) => {
  ...
  const updateTopicTask = (topicTask: ITopicTask) => {
    const topicTaskIndex = topicTasks.findIndex(
      (item) => item.topicTaskId === topicTask.topicTaskId
    );
  ...
  const deleteTopicTask = (topicTaskId: number) => {
    const newTopicTasks = topicTasks.filter(
      (item) => item.topicTaskId !== topicTaskId
    );
    setTopicTasks(newTopicTasks);
  };
